...
#ifdef CONFIG_MACH_RK3026_ESP
extern int rk_gpio_request(int gpio, int direction, int value, const char *label);
...
#if defined(CONFIG_RK3028_105H_KEYS) //jzx@20131216 for 105h 10 point touch.
...
static u16 new_chip_flag = 0;
static u8 chip_type = 0;
...
#ifdef CONFIG_MACH_RK3026_ESP
//	gpio_free(WAKE_PORT);
	if(rk_gpio_request(WAKE_PORT,0, 1, "bl_en") < 0){
#else
	//printk("jzx::%s\n", __func__);
	if(gpio_request(WAKE_PORT,NULL) != 1){
#endif		
		gpio_free(WAKE_PORT);
		printk("zhongchu_init_platform_hw gpio_request wake_port error\n");
		return -EIO;
	}
...
#ifdef CONFIG_MACH_RK3026_ESP
	gpio_free(IRQ_PORT);
	if(rk_gpio_request(IRQ_PORT,1, 1, "tp_int") < 0){
#else
	if(gpio_request(IRQ_PORT,NULL) != 0){
#endif
		gpio_free(IRQ_PORT);
		printk("zhongchu_init_platform_hw gpio_request irq_port error\n");
		return -EIO;
	}
...
#ifdef CONFIG_MACH_88V_FLASH_PCBA40
static void vcc_tp_control(int on)
{
...
#if defined(CONFIG_MACH_88V_FLASH_PCBA40) //|| defined(CONFIG_MACH_RK3026_ESP)
			...
			const struct i2c_device_id *id)
{
	...
	rc=  request_irq(ts->irq, gsl_ts_irq, IRQF_TRIGGER_RISING, client->name, ts);//IRQF_TRIGGER_RISING//irq_flags | IRQF_ONESHOT
	if (rc < 0) {
